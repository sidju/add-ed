...
impl std::fmt::Display for EdError {
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    ...
    match self {
      ...
      HistoryIndexNegative{relative_undo_limit} => write!(f,
      ...
      HistoryIndexSpecialAfterStart{prior_index, special_index} => write!(f,
        "Special history index character `{}` found after history index `{}`.",
        special_index,
        prior_index,
      ),
      HistoryIndexNotInt(text) => write!(f,
        "Failed to parse given history index string `{}` as a number.",
        text,
      ),
      HistoryOffsetNotInt(text) => write!(f,
        "Failed to parse given history index offset `{}` as a number.",
        text,
      ),
      HistoryIndicesUnrelated{prior_index, unrelated_index} => write!(f,
        "Received non-chainable history index `{}` immediately after index `{}`.",
        unrelated_index,
        prior_index,
      ),
      HistoryIndexUnfinished(text) => write!(f,
        "Special index unfinished: `{}`.",
        text,
      ),
